...
from video.deep_3drecon.util.mesh_renderer import MeshRenderer
...
class SECC_Renderer(nn.Module):
    def __init__(self, rasterize_size=None, device="cuda"):
        super().__init__()
        self.face_model = ParametricFaceModel('video/deep_3drecon/BFM')
        ...
        face_feat = np.load("video/deep_3drecon/ncc_code.npy", allow_pickle=True)
        self.face_feat = torch.tensor(face_feat.T).unsqueeze(0).to(device=device)

        del_index_re = np.load('video/deep_3drecon/bfm_right_eye_faces.npy')
        del_index_re = del_index_re - 1
        del_index_le = np.load('video/deep_3drecon/bfm_left_eye_faces.npy')
