...
from video.deep_3drecon.deep_3drecon_models.bfm import perspective_projection


class Face3DHelper(nn.Module):
    def __init__(self, bfm_dir='video/deep_3drecon/BFM', keypoint_mode='lm68', use_gpu=True):
        super().__init__()
    ...
    def load_3dmm(self):
        ...
        if self.keypoint_mode == 'mediapipe':
            self.register_buffer('key_points', torch.from_numpy(np.load("video/deep_3drecon/BFM/index_mp468_from_mesh35709.npy").astype(np.int64)))
...
if __name__ == '__main__':
    ...
    face_mesh_helper = Face3DHelper('video/deep_3drecon/BFM')
